...
export const getPoster = async (req, res, config) => {
  const { address, tokenId, date } = req.query;
  ...
  return res.send(`
    ...
    address.length
                ...
                src="${qrCode}"
                alt=""
                class="aspect-square w-[80rem] h-[80rem] -translate-y-[7rem]"
                />
            </div>
...
export const createPrintOrder = async (req, res, config) => {
  ...
  try {
    ...
    await page.goto(
      `http://localhost:${
        CONFIG.backend.port
      }/api/print/poster?address=${address}&tokenId=${(
        "0000" + token.id
      ).substr(token.id.toString().length)}&date=${formatDate(
        new Date(token.createdAtTimestamp * 1000)
      )}&url=${CONFIG.ipfsBaseUrl}${token.imageURI.replace("ipfs://", "")}`,
